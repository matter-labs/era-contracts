...
The upgrade that is incoming is v33 (an in-place v31 → v33 upgrade; only ZKsync OS chains are
supported). Its per-chain upgrade contract is:

- V33UpgradeZKsyncOS.sol (extends DefaultUpgradeZKsyncOS; no Era counterpart)
...
Additionally, the v33 upgrade of a **ZKsync OS** chain (`V33UpgradeZKsyncOS`) assumes
