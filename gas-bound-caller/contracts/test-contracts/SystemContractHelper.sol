...
import {ThereAreOnly10AccessibleRegisters, FailedToChargeGas} from "gas-bound-caller/contracts/test-contracts/GasBoundCallerErrors.sol";
...
library SystemContractHelper {
    ...
    function getExtraAbiData(uint256 index) internal view returns (uint256 extraAbiData) {
        if (index >= 10) {
            revert ThereAreOnly10AccessibleRegisters();
        }
