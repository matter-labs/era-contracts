...
//! (boojum). The inner Plonk verifier is auto-generated by snarkJS during
//! `ziskup setup_snark` (at `~/.zisk/provingKeySnark/final/PlonkVerifier.sol`)
//! and is regenerated whenever the circuit changes: the adapt step below
//! writes it to a gitignored location for local builds and standalone
//! deployment, and `ZiskVerifier` references the deployed instance by
//! address through `ISnarkPlonkVerifier`. See
