...
function extractMessagesSent(txRec: any): string[] {
    return txRec.logs
        .filter((log: any) =>
        ...
        .map((log: any) =>
            ethers.utils.defaultAbiCoder.decode(['bytes'], log.data)[0]
        );
}

program
    .name('protocol-upgrade')
    .description('CLI tool to start or execute a protocol upgrade')
    .version('1.0.0');
...
program
    ...
    .action(async (options) => {
        try {
            ...
            const decodedUpgradeProposal = ethers.utils.defaultAbiCoder.decode(
                [UPGRADE_PROPOSAL_ABI_TYPE],
                upgradeMsg
            )[0];
