...
import { transferProxyAdminErc20Bridge } from "../src.ts/hyperchain-upgrade";
...
async function main() {
  ...
  program.version("0.1.0").name("transfer-proxy-admin-erc20-bridge").description("transfer the proxy admin role from governarce to proxy admin for l1 erc20 bridge");
  ...
  program
    ...
    .action(async (cmd) => {
      ...
      await transferProxyAdminErc20Bridge(deployer);
    });
