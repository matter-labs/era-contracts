...
contract GatewayCTMOffchainPrepare is Script {
    ...
    /// @notice Some minimal information about the L1->L2 tx
    struct TxInfo {
        address dstAddress;
        bytes data;
        bytes[] factoryDeps;
    ...
    bytes32 constant CREATE2_SALT = bytes32(0);

    function _deployInternal(bytes memory bytecode, bytes memory constructorargs) internal returns (address contractAddress) {
        (bytes32 bytecodeHash, bytes memory deployData) = Utils.getDeploymentCalldata(create2salt, bytecode, constructorargs);

        contractAddress = Utils.getL2AddressViaCreate2Factory(
            CREATE2_SALT,
            bytecodeHash,
            keccak256(constructorargs)
        );

        bytes[] memory deps = new deps[](1);
        deps[0] = bytecode;
