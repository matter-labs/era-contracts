...
abstract contract DeployUtils is Script {
    ...
    function getCreationCalldata(string memory contractName) internal view virtual returns (bytes memory) {
        if (compareStrings(contractName, "ChainRegistrar")) {
            ...
            address _chainTypeManager = addresses.stateTransition.chainTypeManagerProxy;
            return abi.encode(config.ownerAddress, address(0), _chainTypeManager);
            ...
            return abi.encode(restrictions, _chainTypeManager);
