...
contract PrepareZKChainRegistrationCalldataScript is Script {
    ...
    struct Config {
        // Admin of the yet-to-be-registered chain
        address chainAdmin;
        ...
        // It can only be the one that's allowed by the STM. It can be generated by the other scripts or taken from the
    ...
    function initializeConfig() internal {
        ...
        config.bridgehub = IStateTransitionManager(config.stateTransitionProxy).BRIDGE_HUB();
        config.l1SharedBridgeProxy = address(Bridgehub(config.bridgehub).sharedBridge());
        ...
        config.governance = Bridgehub(config.bridgehub).owner();
    ...
    function computeL2BridgeAddress() internal view returns (address) {
        ...
        if (isEOA(config.governance)) {
            l2GovernanceAddress = config.governance;
        } else {
            l2GovernanceAddress = AddressAliasHelper.applyL1ToL2Alias(config.governance);
        }
