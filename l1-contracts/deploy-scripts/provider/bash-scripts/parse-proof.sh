...
modify_json_address() {
    ...
    echo "$json_string" | jq '.result.proof' | sed -e 's/"address":/"addr":/g' -e 's/"type":/"txType":/g' | jq 'walk(if type == "string" and test("^0x[0-9a-fA-F]+$") then if (length < 66) then "0x" + ("0" * (64 - (length - 2))) + .[2:] else . end else . end) | {proof: ., length: (. | length)}'
}
