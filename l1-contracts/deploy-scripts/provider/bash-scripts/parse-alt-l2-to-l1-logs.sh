...
modify_json_address() {
    ...
    echo "$json_string" | jq '.result.l2ToL1Logs | map(del(.logType, .removed, .topics, .data, .isService) | to_entries | sort_by(.key) | from_entries)' | sed -e 's/"address":/"addr":/g' -e 's/"type":/"txType":/g' | jq 'walk(if type == "string" and test("^0x[0-9a-fA-F]+$") then if (length < 66) then "0x" + ("0" * (64 - (length - 2))) + .[2:] else . end else . end) | {l2ToL1Logs: .}'
