...
contract PrepareChainRegistrationPlan is Script {
    ...
    struct Input {
        ...
        address expectedNoDaValidiumL2DaValidator;
        address expectedAvailL2DaValidator;
    }
    ...
    function initialize() internal {
        ...
        input.expectedRollupL2DaValidator = toml.readAddress("$.da.expected_rollup_l2_da_validator_addr");
        input.expectedNoDaValidiumL2DaValidator = toml.readAddress("$.da.expected_no_da_validium_l2_validator_addr");
        input.expectedAvailL2DaValidator = toml.readAddress("$.da.expected_avail_l2_da_validator_addr");
    ...
    function configureZKChain() internal {
        ...
        address l2DaValidator = getL2DAValidator();
        calls[numCalls++] = IChainAdminOwnable.Call({
            target: address(output.diamondProxy),
            value: 0,
    ...
    function getL2DAValidator() internal view returns (address) {
        if (input.daValidatorType == DAValidatorType.Rollup) {
            return input.expectedRollupL2DaValidator;
        } else if (input.daValidatorType == DAValidatorType.NoDA) {
            return input.expectedNoDaValidiumL2DaValidator;
        } else if (input.daValidatorType == DAValidatorType.Avail) {
            return input.expectedAvailL2DaValidator;
        } else {
            revert("Invalid DA validator type");
        }
    }
    ...
    function deployL2DaValidator() internal {
        ...
        output.l2DAValidatorAddress = Utils.deployThroughL1Deterministic({
            ...
            bridgehubAddress: input.bridgehub,
            l1SharedBridgeProxy: derived.assetRouter
        });
        address expectedL2DAValidatorAddress = getL2DAValidator();
        require(expectedL2DAValidatorAddress == output.l2DAValidatorAddress, "L2 DA Validator address mismatch");
    ...
    function getFactoryDeps() internal view returns (bytes[] memory) {
        ...
        // factoryDeps[0] = Utils.readZKFoundryBytecodeL1("L2Bridgehub.sol", "L2Bridgehub");
        // factoryDeps[1] = Utils.readZKFoundryBytecodeL1("L2AssetRouter.sol", "L2AssetRouter");
        // factoryDeps[2] = Utils.readZKFoundryBytecodeL1("L2NativeTokenVaultZKOS.sol", "L2NativeTokenVaultZKOS");
        // factoryDeps[3] = Utils.readZKFoundryBytecodeL1("L2MessageRoot.sol", "L2MessageRoot");
        // factoryDeps[4] = Utils.readZKFoundryBytecodeL1("UpgradeableBeaconDeployer.sol", "UpgradeableBeaconDeployer");
        // factoryDeps[5] = Utils.readZKFoundryBytecodeL1("L2ChainAssetHandler.sol", "L2ChainAssetHandler");
        console.log("=== Bytecode hashes for factoryDeps ===");
        console.log("[!] Compare against zkBytecodeHash in AllContractsHashes.json");
        for (uint256 i = 0; i < factoryDeps.length; i++) {
            ...
            console.log("ZK bytecode hash for factoryDeps [", vm.toString(i), "]:", vm.toString(bytecodeHash));
        }
        console.log("================");
        return factoryDeps;
    }
