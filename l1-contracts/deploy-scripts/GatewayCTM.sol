...
contract GatewaySTM is Script {
    ...
    function deployGatewayFacets(L2ContractsBytecodes memory bytecodes) internal returns (GatewayFacets memory facets) {
        address adminFacet = address(new AdminFacet(config.l1ChainId));
        console.log("Admin facet deployed at", adminFacet);

        address mailboxFacet = address(new MailboxFacet(config.l1ChainId, config.eraChainId));
        console.log("Mailbox facet deployed at", mailboxFacet);
        
        address executorFacet = address(new ExecutorFacet());
        console.log("ExecutorFacet facet deployed at", executorFacet);
        
        address gettersFacet = address(new GettersFacet());
    ...
    function deployGatewayVerifier(L2ContractsBytecodes memory bytecodes) internal returns (address verifier) {
        if (config.testnetVerifier) {
            verifier = address(new TestnetVerifier());
        } else {
    ...
    function deployValidatorTimelock(L2ContractsBytecodes memory bytecodes) internal returns (address validatorTimelock) {
        ...
        validatorTimelock = address(new ValidatorTimelock(deployerAddress, 0, config.eraChainId));
    ...
    function deployGatewayStateTransitionManager() internal {
        ...
        address dp = address(new DiamondProxy());
        console.log("Dummy diamond proxy deployed at", dp);

        output.gatewayStateTransition.stateTransitionImplementation = address(new StateTransitionManager(L2_BRIDGEHUB_ADDRESS));
        ...
        output.gatewayStateTransition.stateTransitionProxy = new TransparentUpgradeableProxy(stmImpl, deployerAddress, abi.encodeCall(StateTransitionManager.initialize, (diamondInitData)));
