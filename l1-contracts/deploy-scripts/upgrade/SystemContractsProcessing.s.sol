...
library SystemContractsProcessing {
    ...
    function getBaseListOfDependencies(bool _isZKsyncOS) internal view returns (bytes[] memory factoryDeps) {
        if (_isZKsyncOS) {
            ...
            // are Era-VM concepts.
            //
            // Additional baselines, none in the L2EcosystemContract enum:
            ...
            // getRemovedTrackerNeutralizations) — not a L2EcosystemContract, so published here.
    ...
    {
        return TransitionDerivationLib.deriveL2DeploymentsFromTable(buildL2BytecodeInfoTable(true), true);
    }
    ...
    ///      - L2WrappedBaseToken: upgrades must not touch the impl on either VM (since v31).
