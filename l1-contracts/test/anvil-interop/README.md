...
├── run-fork-upgrade-test.ts       # Ecosystem upgrade rehearsal against a forked network
...
- **Temporary upgrade inputs**: the upgrade harness copies config inputs into `test/anvil-interop/outputs/upgrade-harness-inputs/` and passes them to Forge via env overrides. It no longer mutates checked-in `upgrade-envs/.../local.toml`.
