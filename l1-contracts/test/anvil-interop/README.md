...
- **v31 -> v32 upgrade harness**: `run-v31-to-v32-upgrade-test.ts` clears the genesis-upgrade tx hash the fixture's chains still carry, which a real chain's server clears once it processes the batch and which otherwise blocks a new upgrade with `PreviousUpgradeNotFinalized`. The fixed v31 scenario does not patch batch counters or backfill state: it asserts that committed and executed batch counts match, requires the fixture's existing base-token flag and caught-up priority queue, and records the lower bound through the real registry. The generic fork runner retains storage-backed fallbacks for source states whose missing history cannot be replayed on Anvil.
