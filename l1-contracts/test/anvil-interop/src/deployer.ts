...
export class ForgeDeployer {
  ...
  async deployL1Core(): Promise<CoreDeployedAddresses> {
    ...
    const scriptPath = "deploy-scripts/DeployL1CoreContracts.s.sol:DeployL1CoreContractsScript";
    ...
    // Use runDeployOnly() to skip acceptAdmin — handled separately via cast
    ...
    return {
      ...
      stateTransitionManager: (deployed as Record<string, unknown>).state_transition_manager_proxy_addr as string,
      validatorTimelock: (deployed as Record<string, unknown>).validator_timelock_addr as string,
      ...
      l1AssetTracker: bridgehub.chain_asset_handler_proxy_addr as string,
      ...
      blobVersionedHashRetriever: (deployed as Record<string, unknown>).blob_versioned_hash_retriever_addr as string,
  ...
  async deployCTM(bridgehubAddr: string): Promise<CTMDeployedAddresses> {
    ...
    const scriptPath = "deploy-scripts/DeployCTM.s.sol:DeployCTMScript";
    // V29: CTM deploy shares the same config/output files as L1 core deploy
    const configPath = "/test/anvil-interop/config/l1-deployment.toml";
    const outputPath = "/test/anvil-interop/outputs/l1-core-output.toml";
    ...
    return {
      ...
      validiumL1DAValidator: ((deployedAddresses.da_addresses as Record<string, unknown>)?.no_da_validium_l1_da_validator_addr ||
        output.validium_l1da_validator) as string,
      rollupL1DAValidator: ((deployedAddresses.da_addresses as Record<string, unknown>)?.rollup_da_manager_addr || output.rollup_l1da_validator) as string,
    };
  }
  ...
  async registerCTM(bridgehubAddr: string, ctmAddr: string): Promise<void> {
    ...
    const scriptPath = "deploy-scripts/RegisterCTM.s.sol:RegisterCTM";
