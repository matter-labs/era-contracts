...
7. **L2 upgrade execution**: The bootloader includes the L2 upgrade tx as a system transaction.
   It calls `ComplexUpgrader.forceDeployAndUpgradeUniversal()`, the only entry point generated by
   this release, which:
...
The v31 ComplexUpgrader already supports `forceDeployAndUpgradeUniversal`, so its existing
...
running v31 delegatecall frame then finishes the remaining entries and the upgrade delegatecall.
...
bytecode deployer and then delegatecalls to `L2V32Upgrade.upgrade()`. That deployer requires the
...
- Production: L2 execution has two stages:
  ...
  2. `L2V32Upgrade.upgrade()` then calls `performForceDeployedContractsInit(false)` to initialize
...
- Production: `L2V32Upgrade.upgrade()` calls `L2BaseToken.initL2(l1ChainId)`.
  On ZKsyncOS, `L2BaseToken.initL2()` calls `MINT_BASE_TOKEN_HOOK`.
