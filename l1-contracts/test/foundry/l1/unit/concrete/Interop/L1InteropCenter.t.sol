...
contract ReentrantL1CrossChainSender is IL1CrossChainSender {
    ...
    constructor(IL1InteropCenter _center) {
        CENTER = _center;
    }
...
contract L1InteropCenterTest is ExperimentalBridgeTestBase {
    ...
    function test_requestTransactionIndirectChecksMagicValue(
        ...
        L1L2IndirectMessageParams memory l2TxnReq2BridgeOut = _createMockL2TransactionRequestIndirectOuter({
            chainId: chainId,
            mintValue: mintValue,
            l2Value: l2Value,
            ...
            indirectCallValue: indirectCallValue,
        ...
        uint256 callerMsgValue = l2TxnReq2BridgeOut.mintValue + l2TxnReq2BridgeOut.indirectCallValue;
        address randomCaller = makeAddr("RANDOM_CALLER");
    ...
    function test_sendMessage_indirect_forwardsAndConfirmsExactRequest() public {
        _useMockSharedBridge();
        _initializeBridgehub();
        ...
        uint256 msgValue = request.mintValue + request.indirectCallValue;
    ...
    function test_sendBundle_indirect_forwardsAndConfirmsExactRequest() public {
        ...
        callAttributes[1] = abi.encodeCall(IERC7786Attributes.indirectCall, (request.indirectCallValue));
        InteropCallStarter[] memory calls = new InteropCallStarter[](1);
        calls[0] = InteropCallStarter({
            to: InteroperableAddress.formatEvmV1(request.crossChainSender),
            data: request.indirectCallData,
    ...
    function _expectIndirectCalls(
        ...
        vm.expectCall(
            crossChainSender,
            _request.indirectCallValue,
            abi.encodeCall(
                IL1CrossChainSender.initiateIndirectCall,
                (_request.chainId, _caller, _request.l2Value, _request.indirectCallData)
            )
        );
        vm.expectCall(
            address(mockChainContract),
    ...
    function test_reentrancyGuardCoversBothSendSurfaces() public {
        ...
        attrs[0] = abi.encodeCall(IERC7786Attributes.l1ToL2TransactionParams, (0, 0, 0, address(this)));
        attrs[1] = abi.encodeCall(IERC7786Attributes.indirectCall, (0));
        vm.expectRevert(Reentrancy.selector);
        l1InteropCenter.sendMessage(InteroperableAddress.formatEvmV1(chainId, address(hook)), hex"", attrs);
        InteropCallStarter[] memory calls = new InteropCallStarter[](1);
        calls[0].to = InteroperableAddress.formatEvmV1(address(hook));
        calls[0].callAttributes = new bytes[](1);
        calls[0].callAttributes[0] = attrs[1];
        bytes[] memory bundleAttrs = new bytes[](1);
        bundleAttrs[0] = attrs[0];
        vm.expectRevert(Reentrancy.selector);
        l1InteropCenter.sendBundle(InteroperableAddress.formatEvmV1(chainId), calls, bundleAttrs);
        bytes32 hash = keccak256("request after reverted reentrancy");
        vm.mockCall(
            address(mockChainContract),
            abi.encodeWithSelector(mockChainContract.bridgehubRequestL2Transaction.selector),
            abi.encode(hash)
        ...
        assertEq(
            l1InteropCenter.sendMessage(InteroperableAddress.formatEvmV1(chainId, mockL2Contract), hex"", bundleAttrs),
            hash
        );
    }
    ...
    function testFuzz_exactFunding(bool _ethBase, bool _indirect, uint96 _mintValue, uint96 _indirectValue) public {
        ...
        uint256 totalValue = (_ethBase ? mintValue : 0) + indirectValue;
        bytes32 canonicalHash = keccak256("canonicalHash");
        bytes[] memory attributes = new bytes[](_indirect ? 3 : 2);
        attributes[0] = abi.encodeCall(IERC7786Attributes.l1ToL2TransactionParams, (mintValue, 1_000_000, 800, caller));
        attributes[1] = abi.encodeCall(IERC7786Attributes.interopCallValue, (0));
        if (_indirect) {
            attributes[2] = abi.encodeCall(IERC7786Attributes.indirectCall, (indirectValue));
        }
        // Isolate transport accounting from token custody; the integration tests exercise real custody.
        vm.mockCall(
            sharedBridgeAddress,
            abi.encodeWithSelector(IAssetRouterShared.bridgehubDepositBaseToken.selector),
            hex""
        ...
        vm.expectCall(
            sharedBridgeAddress,
            _ethBase ? mintValue : 0,
            abi.encodeCall(
                IAssetRouterShared.bridgehubDepositBaseToken,
                (chainId, bridgehub.baseTokenAssetId(chainId), caller, mintValue)
            )
        );
        vm.mockCall(
            address(mockChainContract),
        ...
        if (_indirect) {
            IndirectCallRequest memory output = IndirectCallRequest({
                magicValue: INDIRECT_CALL_MAGIC_VALUE,
                l2Contract: mockL2Contract,
                l2Calldata: hex"1234",
                factoryDeps: new bytes[](0),
                txDataHash: keccak256("depositData")
            });
            vm.mockCall(
                crossChainSender,
                abi.encodeWithSelector(IL1CrossChainSender.initiateIndirectCall.selector),
            ...
            vm.expectCall(
                crossChainSender,
                indirectValue,
                abi.encodeCall(IL1CrossChainSender.initiateIndirectCall, (chainId, caller, 0, hex"1234"))
            );
            vm.expectCall(
                crossChainSender,
                abi.encodeCall(
                    IL1CrossChainSender.confirmL2Transaction,
                    (chainId, keccak256("depositData"), canonicalHash)
                )
            );
        }
        bytes memory recipient = InteroperableAddress.formatEvmV1(
            chainId,
            _indirect ? crossChainSender : mockL2Contract
        );
        vm.deal(caller, totalValue + 1);
        vm.prank(caller, caller);
        assertEq(l1InteropCenter.sendMessage{value: totalValue}(recipient, hex"1234", attributes), canonicalHash);
        assertEq(caller.balance, 1);
        vm.deal(caller, totalValue + 1);
        vm.expectRevert(abi.encodeWithSelector(MsgValueMismatch.selector, totalValue, totalValue + 1));
        vm.prank(caller, caller);
        l1InteropCenter.sendMessage{value: totalValue + 1}(recipient, hex"1234", attributes);
    }
}
