...
import {IndexedMerkleTree, IMTLeaf} from "contracts/common/libraries/IndexedMerkleTree.sol";
...
contract L2InteropCommitmentTreeTest is Test {
    ...
    function test_insert_emitsRootPublished() public {
        ...
        // the deadline anchor is the settlement-layer timestamp `t`, derived from the proof's settlement metadata). The
    ...
    function test_insert_inclusionPathVerifies() public {
        ...
        assertTrue(
            IndexedMerkleTree.verifyInclusion(tree.root(), 200, leaf, idx, path),
    ...
    function test_insert_selfHealsStaleLowLeaf() public {
        tree.insert(100, 0); // head -> 100 (leaf 1); head.nextValue == 100
        // 150's true low-nullifier is leaf 1 (100), but we pass the stale head index 0 (head.next == 100
        // <= 150). The dynamic-height IMT no longer reverts: it self-heals by walking forward from the
        // supplied low leaf to the correct predecessor, so the insert succeeds and stays sorted.
        (uint256 idx, ) = tree.insert(150, 0);
        assertEq(idx, 2, "150 inserted at the next index despite the stale low-leaf hint");
        assertEq(AtomicInteropTestUtils.indexOfValue(tree, 150), 2, "150 is present in the tree");
        IMTLeaf memory leaf100 = tree.leafAt(1);
        assertEq(leaf100.nextValue, 150, "sorted linked list maintained: 100 -> 150 after self-heal");
    }
}
