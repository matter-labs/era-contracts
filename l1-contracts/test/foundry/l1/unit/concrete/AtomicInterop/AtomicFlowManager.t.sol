...
import {LegState, ImtProof, AtomicTimeoutProof, AtomicFinalityProof} from "contracts/atomic-interop/IAtomicInterop.sol";
...
// The dynamic-height IMT (upstream) reverts this from `verifyNonInclusion` when a low leaf cannot
// bracket the value (e.g. the value IS present), instead of the old return-false path.
import {IMTLowLeafNextTooSmall} from "contracts/common/L1ContractErrors.sol";
...
/// scope; it is exercised end-to-end in the separate anvil-interop suite). The deadline is an
/// **SL timestamp**: {AtomicInteropProof} re-parses the SAME `messageProof` bytes with the real
/// {MessageHashing._getProofData} (NOT mocked) to derive each batch's settlement timestamp `t`, so the
/// proof bytes here are format-valid multi-hop proofs carrying a CHOSEN `t`
/// ({AtomicInteropTestUtils.slProofBytes}). Inclusion requires `t <= deadline`; the timeout is an
/// adjacency proof — absence at the last batch `N` with `t_N <= deadline`, pinned by the consecutive
/// successor `N+1` with `t_{N+1} > deadline`. `messageIndex` is 0 because the minimal proof has a
/// zero-length log-leaf path.
///
...
contract AtomicFlowManagerTest is Test {
    ...
    /// @dev The SL snapshot block carried in the proof. The atomic deadline is now compared to the
    /// batch's settlement timestamp `t` (not this block), so its exact value is irrelevant here.
    uint256 internal constant DUMMY_SL_BLOCK = 1;
    ...
    function test_requireFlowFinalized_revertsOnFlowIdMismatch() public {
        ...
        bytes32 computed = AtomicInteropTestUtils.computeFlowId(
            finality.legBundleHashes,
            finality.legSourceChainIds,
            DEADLINE,
            SL_CHAIN_ID
        );
    ...
    function test_requireFlowFinalized_revertsForNonInteropHandler() public {
        ...
        AtomicFinalityProof memory finality = AtomicFinalityProof({
            flowId: bytes32(0),
            deadline: DEADLINE,
            settlementLayerChainId: SL_CHAIN_ID,
    ...
    function test_refund_happyTimeoutPath() public {
        ...
        // 1. Authorize the refund: adjacency timeout for the missing leg against B's tree —
        //    absence at the last batch with t <= deadline, pinned by a successor batch with t > deadline.
        AtomicTimeoutProof memory timeout = _timeout(treeB, CHAIN_B, flowId, missingLeg);
    ...
    function test_claimRefund_revertsWhenACallNotRecovered() public {
        // A Revertable leg whose bundle call reports "nothing recovered" -> ManagerCallNotRecovered
        // (every fund-moving call must recover; a single non-recovered call must not silently strand funds).
        ...
        AtomicTimeoutProof memory timeout = _timeout(treeB, CHAIN_B, flowId, hBA);
        managerA.authorizeRefund(
            flowId,
            _legHashes(hAB, hBA),
            _legSourceChainIds(hAB, hBA),
        ...
        // The single call (index 0) does not recover -> the whole claim reverts (no partial / silent strand).
        vm.expectRevert(abi.encodeWithSelector(ManagerCallNotRecovered.selector, flowId, emptyHash, uint256(0)));
        managerA.claimRefund(flowId, emptyBundleBytes);
    }

    function test_authorizeRefund_revertsWhenSuccessorWithinDeadline() public {
        // A3: the adjacency successor must be PAST the deadline. If batch N's successor still has
        // `t <= deadline` (here 1800 <= 2000), then N is NOT the last in-time batch — the value could
        // still be committed between N and the deadline — so the refund is rejected. This is exactly what
        // defeats the stale/genesis-root force-refund.
        bytes32 hAB = _bundleHashAB();
        bytes32 hBA = _bundleHashBA();
        bytes32 flowId = _flowId(hAB, hBA);
        _appendLeg(managerA, treeA, flowId, _sourceLegOf(CHAIN_A, hAB, hBA));
        ...
        // Absence at batch 1 (t_N = 1500 <= deadline, valid A2) but successor batch 2 has t = 1800, still
        // <= deadline -> A3 fails.
        AtomicTimeoutProof memory timeout = AtomicTimeoutProof({
            absence: _absence(treeB, CHAIN_B, flowId, missingLeg, 1, 1500),
            successor: _successor(treeB, CHAIN_B, 2, 1800)
        });

        vm.expectRevert(abi.encodeWithSelector(ProofDeadlineNotExceeded.selector, uint256(1800), DEADLINE));
        managerA.authorizeRefund(
            flowId,
            _legHashes(hAB, hBA),
            _legSourceChainIds(hAB, hBA),
    ...
    function test_authorizeRefund_revertsWhenSuccessorNotConsecutive() public {
        // A3: the adjacency witness MUST be the consecutive successor (batch N+1). A non-adjacent
        // batch (here 3, not 2) cannot pin N=1 as the last in-time batch -> ProofAdjacencyNotConsecutive.
        bytes32 hAB = _bundleHashAB();
        bytes32 hBA = _bundleHashBA();
        bytes32 flowId = _flowId(hAB, hBA);
        _appendLeg(managerA, treeA, flowId, _sourceLegOf(CHAIN_A, hAB, hBA));
        ...
        AtomicTimeoutProof memory timeout = AtomicTimeoutProof({
            absence: _absence(treeB, CHAIN_B, flowId, missingLeg, 1, 1500),
            successor: _successor(treeB, CHAIN_B, 3, 2100) // batch 3 is not the consecutive successor of 1
        });

        vm.expectRevert(abi.encodeWithSelector(ProofAdjacencyNotConsecutive.selector, uint256(1), uint256(3)));
        managerA.authorizeRefund(
            flowId,
            _legHashes(hAB, hBA),
            _legSourceChainIds(hAB, hBA),
    ...
    function test_authorizeRefund_revertsWhenAbsenceBatchPastDeadline() public {
        // A2: the absence batch N must itself be in time (t_N <= deadline). An absence proof at a
        // batch already past the deadline does not bound the value's commit window -> ProofDeadlineExceeded.
        bytes32 hAB = _bundleHashAB();
        bytes32 hBA = _bundleHashBA();
        bytes32 flowId = _flowId(hAB, hBA);
        _appendLeg(managerA, treeA, flowId, _sourceLegOf(CHAIN_A, hAB, hBA));
        ...
        AtomicTimeoutProof memory timeout = AtomicTimeoutProof({
            absence: _absence(treeB, CHAIN_B, flowId, missingLeg, 1, 2500), // t_N = 2500 > deadline
            successor: _successor(treeB, CHAIN_B, 2, 3000)
        });

        vm.expectRevert(abi.encodeWithSelector(ProofDeadlineExceeded.selector, uint256(2500), DEADLINE));
        managerA.authorizeRefund(
            flowId,
            _legHashes(hAB, hBA),
            _legSourceChainIds(hAB, hBA),
    ...
    function test_authorizeRefund_revertsIfLegActuallyPresent() public {
        // Both legs committed: the "missing" leg IS present on B. Using the head as its low leaf, the
        // head's next value equals the present commit value, so the IMT's non-membership check rejects it
        // -> `IMTLowLeafNextTooSmall` (the dynamic-height IMT reverts here rather than returning false).
        // The A2 deadline check runs first and passes (t = 1500 <= DEADLINE); the successor would satisfy
        // A3, but the non-membership revert fires first. Either way the present leg cannot be refunded.
        bytes32 hAB = _bundleHashAB();
        bytes32 hBA = _bundleHashBA();
        bytes32 flowId = _flowId(hAB, hBA);
        _appendLeg(managerA, treeA, flowId, _sourceLegOf(CHAIN_A, hAB, hBA));
        ...
        AtomicTimeoutProof memory timeout = AtomicTimeoutProof({
            // Absence "proof" against batch 1 (t = 1500 <= deadline) but the head leaf does not bracket a
            // present value, so the non-membership check fails.
            absence: ImtProof({
                ...
                messageProof: AtomicInteropTestUtils.slProofBytes(DUMMY_SL_BLOCK, SL_CHAIN_ID, 1500),
        ...
        // legB is the smallest value in B's tree, so head.nextValue == legBValue == value.
        vm.expectRevert(abi.encodeWithSelector(IMTLowLeafNextTooSmall.selector, legBValue, legBValue));
        managerA.authorizeRefund(
            flowId,
            _legHashes(hAB, hBA),
            _legSourceChainIds(hAB, hBA),
    ...
    function test_mutualExclusivity_refundedThenFinalizeBlocked() public {
        ...
        // A commits; B never commits its leg. Refund A's source leg via the adjacency timeout.
        _appendLeg(managerA, treeA, flowId, sourceLegA);
        AtomicTimeoutProof memory timeout = _timeout(treeB, CHAIN_B, flowId, missingLeg);
        managerA.authorizeRefund(
            flowId,
    ...
    /// @dev Per-leg source chain ids, POSITIONAL — aligned 1:1 with `_legHashes(_hAB, _hBA)`.
    /// The AB leg is sourced on A, the BA leg on B; the order follows the sorted leg-hash order, so the
    /// result need not be ascending (it is whatever the hash sort yields).
    function _legSourceChainIds(bytes32 _hAB, bytes32 _hBA) internal pure returns (uint256[] memory ids) {
        bytes32[] memory hashes = _legHashes(_hAB, _hBA);
        ids = new uint256[](2);
        ids[0] = hashes[0] == _hAB ? CHAIN_A : CHAIN_B;
        ids[1] = hashes[1] == _hAB ? CHAIN_A : CHAIN_B;
    }

    /// @dev flowId from the two leg hashes + positional source chains + deadline + the flow's single SL.
    function _flowId(bytes32 _hAB, bytes32 _hBA) internal pure returns (bytes32) {
        return
            AtomicInteropTestUtils.computeFlowId(
                _legHashes(_hAB, _hBA),
                _legSourceChainIds(_hAB, _hBA),
                DEADLINE,
                SL_CHAIN_ID
            );
    }
    ...
    /// order), each carrying batch settlement timestamp `_slBlock` (the SL-assigned `t` value). Requires both legs already appended to their trees.
    function _finality(
        bytes32 _flowIdLocal,
        bytes32 _hAB,
        bytes32 _hBA,
        ...
        finality = AtomicFinalityProof({
            ...
            legSourceChainIds: _legSourceChainIds(_hAB, _hBA),
            proofs: proofs
        });
    }
    ...
    function _partialFinality(
        ...
        ImtProof[] memory proofs = new ImtProof[](2);
        for (uint256 i = 0; i < 2; ++i) {
            // Both proofs reference A's tree and the present leg's leaf; only the settlement timestamp (> deadline)
    ...
    /// state, carrying batch settlement timestamp `_batchTimestamp` in its message-proof bytes.
    function _inclusion(
        bytes32 _flowIdLocal,
        bytes32 _bundleHash,
        uint256 _batchTimestamp
        ...
        return
            ImtProof({
                ...
                messageTxNumberInBatch: DUMMY_TX_IN_BATCH,
                messageIndex: DUMMY_MSG_INDEX,
                messageProof: AtomicInteropTestUtils.slProofBytes(DUMMY_SL_BLOCK, SL_CHAIN_ID, _batchTimestamp),
    ...
    /// @dev Absence proof: non-inclusion of `(flowId, bundleHash)` against `_tree`, claimed to be
    /// the last batch `_batchNumber` with settlement timestamp `_t` (`_t <= deadline` for a valid A2).
    function _absence(
        ...
        return
            ImtProof({
                ...
                messageTxNumberInBatch: DUMMY_TX_IN_BATCH,
                messageIndex: DUMMY_MSG_INDEX,
                messageProof: AtomicInteropTestUtils.slProofBytes(DUMMY_SL_BLOCK, SL_CHAIN_ID, _t),
    ...
    /// @dev Successor witness: a root-authentication proof of batch `_batchNumber` of `_chainId`
    /// with settlement timestamp `_t` (`_t > deadline` pins the predecessor as the last in-time batch).
    /// Its IMT membership fields are unused by {AtomicInteropProof.verifyTimeoutAdjacency}.
    function _successor(
        IL2InteropCommitmentTree _tree,
        uint256 _chainId,
        uint256 _batchNumber,
        ...
        return
            ImtProof({
                ...
                leaf: _tree.leafAt(0),
                imtLeafIndex: 0,
                imtProof: _tree.merklePath(0)
            });
    }

    /// @dev A standard, valid adjacency timeout for `_bundleHash` on `_tree`/`_chainId`: absence at
    /// batch 1 (`t_N = 1500 <= deadline`) pinned by the consecutive successor batch 2 (`t = 2100 > deadline`).
    function _timeout(
        IL2InteropCommitmentTree _tree,
        uint256 _chainId,
        bytes32 _flowIdLocal,
        bytes32 _bundleHash
    ) internal view returns (AtomicTimeoutProof memory) {
        return
            AtomicTimeoutProof({
                absence: _absence(_tree, _chainId, _flowIdLocal, _bundleHash, 1, 1500),
                successor: _successor(_tree, _chainId, 2, 2100)
            });
    }
}
