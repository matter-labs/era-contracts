...
abstract contract SharedL2ContractDeployer is UtilsCallMockerTest, DeployIntegrationUtils {
    ...
    function _readTotalWithdrawalsToL1(bytes32 _assetId) internal view returns (uint256) {
        return L2NativeTokenVault(L2_NATIVE_TOKEN_VAULT_ADDR).interopInfo(_assetId).totalWithdrawalsToL1;
    }

    function _readTotalSuccessfulDepositsFromL1(bytes32 _assetId) internal view returns (uint256) {
        return L2NativeTokenVault(L2_NATIVE_TOKEN_VAULT_ADDR).interopInfo(_assetId).totalSuccessfulDepositsFromL1;
    }
