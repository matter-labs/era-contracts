...
abstract contract L2AtomicInteropSendRefundTestAbstract is L2InteropTestUtils, AtomicInteropProofBuilder {
    ...
    /// `recoverBaseToken` and records no bookkeeping). The value-leg refund flows below end
    /// inside the REAL holder — escrow release plus its recoverability invariants — so etch the real
    /// {BaseTokenHolder} over it (same pattern as the bookkeeping flow tests in
    ...
    function _etchRealBaseTokenHolder() internal {
        ...
        // `NTV.originChainId(baseTokenAssetId)` to `block.chainid`, which would trip the holder's
        // `BaseTokenNativeToThisChain` recovery invariant. The real NTV storage holds L1_CHAIN_ID (a
        // base token is never native to its own L2, an invariant `recoverBaseToken` asserts) —
        // re-mock it back to the real value so the holder takes the production non-native branches.
        vm.mockCall(
    ...
    /// @dev Reads the real holder's L1 interop counters (see {BaseTokenHolder.baseTokenInteropInfo}).
    function _holderInteropInfo() internal view returns (uint256 withdrawals, uint256 deposits) {
    ...
    function _sendDirectValueLegWithInvalidRemotePeer(address _depositor) internal {
        ...
        (uint256 withdrawalsBefore, uint256 depositsBefore) = _holderInteropInfo();

        // The escrow leg: the collected value must reach the real BaseTokenHolder's recording entry
        // point with the original (destination, amount).
        vm.expectCall(
            L2_BASE_TOKEN_HOLDER_ADDR,
            NATIVE_VALUE_LEG_AMOUNT,
        ...
        // An L2->L2 bridge-out records nothing in the L1 interop counters (only L1-destined
        // withdrawals are counted; see {BaseTokenHolder.burnAndStartBridging}).
        (uint256 withdrawalsAfter, uint256 depositsAfter) = _holderInteropInfo();
        assertEq(withdrawalsAfter, withdrawalsBefore, "an L2->L2 escrow must not bump totalWithdrawalsToL1");
        assertEq(depositsAfter, depositsBefore, "an L2->L2 escrow must not bump totalSuccessfulDepositsFromL1");
    }
    ...
    function test_atomicSend_directValueLeg_sameBase_timeoutRefundsEscrowedValue() public {
        ...
        // The recovery must reach the holder's `recoverBaseToken` with the original bridge-out
        // destination (so its L2->L2-only recoverability invariants run against the right chain id).
        vm.expectCall(
            L2_BASE_TOKEN_HOLDER_ADDR,
            abi.encodeCall(
        ...
        // An L2->L2 bridge-out recorded nothing at send, so the refund has nothing to reverse: the
        // append-only L1 counters must stay untouched.
        (uint256 withdrawalsAfter, uint256 depositsAfter) = _holderInteropInfo();
        assertEq(withdrawalsAfter, withdrawalsBefore, "the refund must not touch totalWithdrawalsToL1");
        assertEq(depositsAfter, depositsBefore, "the refund must not touch totalSuccessfulDepositsFromL1");
