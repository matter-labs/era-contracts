...
contract DisabledProofSystemsTest is UtilsCallMockerTest {
    ...
    function test_disabled_stillRequiresAirbender() public {
        ...
        bytes memory data = _proofData(genesis, batch);
        vm.prank(validator);
    ...
    function _deployChain(bool _isZKsyncOS, uint256 _chainId) internal returns (address result) {
        ...
        mockChainTypeManagerVerifier(address(verifier));
        vm.mockCall(
    ...
    function _proofData(
        ...
        batches[0] = _batch;
        uint256[] memory proof = new uint256[](3 + ZKSYNC_OS_PROOF_METADATA_LENGTH + ZISK_SNARK_PROOF_LENGTH);
        proof[0] = 5;
    ...
    function _prove(
        ...
        bytes memory data = _proofData(_previous, _batch);
        vm.expectEmit(true, true, false, true, _chain);
