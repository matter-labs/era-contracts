...
///         enum-indexed L2 bytecode table, its shared system-proxy shell and the factory
///         dependencies the prepare publishes — as JSON, for the anvil-interop registry harness.
///         Its bootstrap release is built from this very inventory, so the harness and the
///         production prepare share one code path for the table instead of a TypeScript mirror
///         of it.
contract EmitL2BytecodeInventory is Script {
    function run(string memory _outputPath) external {
        ...
        vm.serializeBytes("inventory", "systemProxyBytecodeInfo", SystemContractsProcessing.systemProxyBytecodeInfo());
