...
contract MultiProofVerifierDeploymentTest is Test {
    ...
    function test_testnetLane_composition() public {
        ...
        assertTrue(
            ZKsyncOSTestnetVerifier(deployer.airbenderVerifier()).isTestnetVerifier(),
    ...
    function test_tooling_introspectsChainVerifier() public {
        deployer.deployMultiProofLane(owner, ziskPlonk, true);
        (, address plonk) = DeployCTML1OrGateway.getSubVerifiers(deployer.chainVerifier());
        assertEq(plonk, deployer.verifierPlonk(), "testnet lane PLONK");
        assertTrue(MultiProofTestnetVerifier(deployer.chainVerifier()).isTestnetVerifier());

        deployer.deployMultiProofLane(owner, ziskPlonk, false);
        (, plonk) = DeployCTML1OrGateway.getSubVerifiers(deployer.chainVerifier());
        assertEq(plonk, deployer.verifierPlonk(), "prod lane PLONK");
        assertFalse(MultiProofVerifier(deployer.chainVerifier()).isTestnetVerifier());
    }
}
