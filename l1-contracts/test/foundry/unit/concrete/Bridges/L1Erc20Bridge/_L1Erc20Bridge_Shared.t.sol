...
contract L1Erc20BridgeTest is Test {
    ...
    constructor() {
        ...
        bridge = new L1ERC20Bridge(IL1AssetRouter(address(dummySharedBridge)), IL1NativeTokenVault(address(1)));
        uint256 eraChainId = 1;
        address weth = makeAddr("weth");
        L1NativeTokenVault ntv = new L1NativeTokenVault(weth, IL1AssetRouter(address(dummySharedBridge)), eraChainId);

        vm.store(address(bridge), bytes32(uint256(212)), bytes32(0));

        reenterL1ERC20Bridge = new ReenterL1ERC20Bridge();
        bridgeReenterItself = new L1ERC20Bridge(IL1AssetRouter(address(reenterL1ERC20Bridge)), ntv);
