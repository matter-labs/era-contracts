...
contract L1SharedBridgeLegacyTest is Test {
    ...
    function setUp() public {
        ...
        TransparentUpgradeableProxy sharedBridgeProxy = new TransparentUpgradeableProxy(address(sharedBridgeImpl), admin, abi.encodeWithSelector(L1SharedBridge.initialize.selector, owner, 0));
    ...
    function test_depositLegacyERC20Bridge() public {
        ...
        vm.mockCall(bridgehubAddress, abi.encodeWithSelector(
            IBridgehub.requestL2TransactionDirect.selector), abi.encode(txHash));

        vm.prank(l1ERC20BridgeAddress);
        sharedBridge.depositLegacyErc20Bridge(alice, bob, address(token), amount, l2TxGasLimit, l2TxGasPerPubdataByte, refundRecipient);
    }
    ...
    function test_finalizeWithdrawalLegacyErc20Bridge_EthOnEth() public {
        ...
        vm.prank(l1ERC20BridgeAddress);
        sharedBridge.finalizeWithdrawalLegacyErc20Bridge(l2BatchNumber, l2MessageIndex, l2TxNumberInBatch, message, merkleProof);
    }
    ...
    function test_finalizeWithdrawalLegacyErc20Bridge_ErcOnEth() public {
        ...
        /// storing chainBalance
        uint256 chainBalanceLocationInStorage = uint256(6 - 1 + 1 + 1 );
        vm.store(address(sharedBridge), keccak256(abi.encode( uint256(uint160(address(token))), keccak256(abi.encode(ERA_CHAIN_ID , chainBalanceLocationInStorage)))), bytes32(amount));
        vm.mockCall(bridgehubAddress, abi.encodeWithSelector(IBridgehub.baseToken.selector), abi.encode(ETH_TOKEN_ADDRESS));
    ...
    function test_claimFailedDepositLegacyErc20Bridge_Erc() public {
        ...
        vm.store(address(sharedBridge), keccak256(abi.encode( txHash , keccak256(abi.encode(ERA_CHAIN_ID , depositLocationInStorage)))), txDataHash);
        ...
        vm.mockCall(bridgehubAddress, abi.encodeWithSelector(IBridgehub.proveL1ToL2TransactionStatus.selector, ERA_CHAIN_ID, txHash, l2BatchNumber, l2MessageIndex,
            l2TxNumberInBatch,
            merkleProof,
        ...
        vm.prank(l1ERC20BridgeAddress);
        sharedBridge.claimFailedDepositLegacyErc20Bridge(alice, address(token), amount, txHash, l2BatchNumber, l2MessageIndex, l2TxNumberInBatch, merkleProof);
    }
}