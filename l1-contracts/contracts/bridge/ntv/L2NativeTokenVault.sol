...
import {IL2NativeTokenVault, InteropL2Info} from "./IL2NativeTokenVault.sol";
...
contract L2NativeTokenVault is IL2NativeTokenVault, NativeTokenVaultBase {
    ...
    /// @notice L2-side accounting of L1 <-> L2 flows. All chains settle on L1, so no settlement-layer
    /// distinction is made. It allows reconstructing the amount of a token attributable to the L1
    /// escrow if per-chain accounting is ever reintroduced.
    ...
    function _handleBridgeToChain(uint256 _chainId, bytes32 _assetId, uint256 _amount) internal virtual override {
        ...
        // All chains settle on L1, so every L1-destined withdrawal is recorded.
    ...
    function _handleBridgeFromChain(uint256 _chainId, bytes32 _assetId, uint256 _amount) internal virtual override {
        ...
        // All chains settle on L1, so every L1-originated deposit is recorded.
        if (_chainId == L1_CHAIN_ID) {
