...
interface INativeTokenVault {
    ...
    /// @notice Returns the bridged token count
    function bridgedTokenCount() external view returns (uint256);

    /// @notice Returns the bridged tokens
    function bridgedTokens(uint256 count) external view returns (bytes32);
