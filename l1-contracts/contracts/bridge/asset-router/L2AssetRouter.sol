...
contract L2AssetRouter is AssetRouterBase, IL2AssetRouter, ReentrancyGuard {
    ...
    /// @dev Note, that while it is a simple storage variable, the name is in capslock for the backward compatibility with
    /// the old version where it was an immutable.
    IL2SharedBridgeLegacy public L2_LEGACY_SHARED_BRIDGE; //@rev
    ...
    modifier onlyAssetRouterCounterpart(uint256 _originChainId) {
        if (_originChainId == L1_CHAIN_ID) {
            ...
            if (AddressAliasHelper.undoL1ToL2Alias(msg.sender) != address(L1_ASSET_ROUTER)) {
    ...
    ) public payable override onlyAssetRouterCounterpartOrSelf(L1_CHAIN_ID) nonReentrant {
    ...
    function _getSharedBridgeWithdrawMessage(
        ...
        address _l1Token,
        uint256 _amount
    ) internal pure returns (bytes memory) { //TODO deprecate, eventually
        // solhint-disable-next-line func-named-parameters
        return abi.encodePacked(IL1ERC20Bridge.finalizeWithdrawal.selector, _l1Receiver, _l1Token, _amount);
    }

    /*//////////////////////////////////////////////////////////////
                            LEGACY FUNCTIONS  //TODO deprecate all the below, eventually
    //////////////////////////////////////////////////////////////*/
    ...
    ) external payable onlyAssetRouterCounterpart(L1_CHAIN_ID) {//@rev remove or keep
    ...
    function finalizeDepositLegacyBridge(
        ...
        uint256 _amount,
        bytes calldata _data
    ) external onlyLegacyBridge { //@rev remove or keep
        _translateLegacyFinalizeDeposit({
            _l1Sender: _l1Sender,
            _l2Receiver: _l2Receiver,
            _l1Token: _l1Token,
