...
contract L1Nullifier is IL1Nullifier, ReentrancyGuard, Ownable2StepUpgradeable, PausableUpgradeable {
    ...
    /// @dev Verifies (via Merkle proof) that the L1 -> L2 transaction reached the claimed status, checks
    /// the recomputed txDataHash against the recorded deposit, and deletes the record — each deposit is
    /// claimable once.
