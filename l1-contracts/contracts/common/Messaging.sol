...
/// @param shadowAccount If true, the InteropHandler routes execution through the sender's ShadowAccount on the destination chain
///                      instead of calling `to` directly. The ShadowAccount is deployed deterministically via the ShadowAccountFactory
///                      and the `data` field is decoded as a ShadowAccountCall[] array by the ShadowAccount.
