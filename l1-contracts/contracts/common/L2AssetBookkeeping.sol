...
/// the tracker's `isAssetTracked` is set for the asset: every writer that marks an asset tracked
/// records the baseline in the same call, so no separate saved-flag is needed. The baseline's exact
/// conventions are defined at the storage site — see `L2AssetTracker.assetBookkeeping`.
