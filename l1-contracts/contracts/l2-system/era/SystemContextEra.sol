 ...
 * @notice Era-specific SystemContext contract. Near-verbatim port of the original
 * system-contracts/contracts/SystemContext.sol.
 * @dev The only structural change from the original is that slot 270
 * (currentSettlementLayerChainId) is now a CommonSystemContextStorage struct, shared
 * with SystemContextZKOS via CommonSystemContextLib. No base contract is used.
 */
contract SystemContextEra is ISystemContextDeprecated {
    ...
    /// @dev Slot 270. The struct wraps currentSettlementLayerChainId so that the shared
    /// setSettlementLayerChainId logic can be delegated to CommonSystemContextLib.
    /// Storage-compatible with the original `uint256 public currentSettlementLayerChainId`.
