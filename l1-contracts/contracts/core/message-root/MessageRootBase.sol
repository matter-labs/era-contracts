...
abstract contract MessageRootBase is IMessageRootBase, ReentrancyGuard, Initializable, MessageVerification {
    ...
    /// @notice The settlement-layer timestamp folded into each `(chainId, batchNumber)` batch leaf,
    /// stored so off-chain proof builders can recover the exact value a proof must carry.
