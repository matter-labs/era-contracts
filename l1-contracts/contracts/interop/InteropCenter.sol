    ...
    PausableUpgradeable
{
    ...
    function _ensureCorrectTotalValue(
        ...
        if (_destinationBaseTokenAssetId == thisChainBaseTokenAssetId) {
            ...
            if (_totalBurnedCallsValue > 0) {
                ...
                // Send tokens to BaseTokenHolder, which records the outbound flow, via burnAndStartBridging
    ...
    function _sendBundle(
        ...
        // `totalWithdrawalsToL1` accounting must stay append-only, see {L2NativeTokenVault.interopInfo}
        // and {BaseTokenHolder.baseTokenInteropInfo}). Checked here, before any burn, since both entry
        // points (`sendMessage` and `sendBundle`) funnel through `_sendBundle`.
