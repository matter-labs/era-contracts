...
contract RelayedSLDAValidator is IL1DAValidator, CalldataDA {
    ...
    function checkDA(
        ...
        if (pubdataSource == uint8(PubdataSource.Calldata)) {
            ...
            (blobCommitments, pubdata) = _processCalldataDA(blobsProvided, fullPubdataHash, _maxBlobsSupported, l1DaInput[1:]);

            // Re-sending all the pubdata in pure form to L1.
            IL1Messenger(L2_TO_L1_MESSENGER_SYSTEM_CONTRACT_ADDR).sendToL1(abi.encode(
                _chainId,
                pubdata
            ));
