...
contract MailboxFacet is ZKChainBase, IMailbox {
    ...
    function _requestL2Transaction(WritePriorityOpParams memory _params) internal returns (bytes32 canonicalTxHash) {
        ...
        if (block.chainid == L1_CHAIN_ID) {
            is7702AccountSender = EIP_7702_CHECKER.isEIP7702Account(request.sender); // This is not the same as refundRecipient, as it appears to be the AR during the indirect-call flow.
