...
contract CommitterFacet is ZKChainBase, ICommitter {
    ...
    function _commitOneBatchEra(
        ...
        if (s.priorityModeInfo.activated && (logOutput.numberOfLayer2Txs != 0 || logOutput.numberOfLayer1Txs == 0)) {
            revert InvalidTxCountInPriorityMode(logOutput.numberOfLayer2Txs, logOutput.numberOfLayer1Txs);
        }
    ...
    function _commitOneBatchZKsyncOS(
        ...
        if (_newBatch.slChainId != block.chainid) {
            revert IncorrectBatchSLChainId(_newBatch.slChainId, block.chainid);
        }
