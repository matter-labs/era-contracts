...
contract AdminFacet is ZkSyncStateTransitionBase, IAdmin {
    ...
    function setValidiumMode(PubdataPricingMode _validiumMode) external onlyAdmin {
        require(s.storedBatchHashes[1] == 0x00, "AdminFacet: set validium only after genesis"); // Validium mode can be set only before the first batch is committed        s.feeParams.pubdataPricingMode = _validiumMode;
