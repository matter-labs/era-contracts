...
- `aggregatorProgramVK` — the programVK of the aggregator guest ELF. The
  ...
  it, put the four limbs into `tools/data/ZiSK_vk.json`, and regenerate the
  contract.
...
`tools/data/ZiSK_plonk_verification_key.json`, so the two steps below run
...
1. From `tools/`, render the snarkJS Plonk verifier from the committed key.
