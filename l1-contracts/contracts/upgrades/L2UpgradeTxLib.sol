...
import {IL2V33Upgrade} from "./IL2V33Upgrade.sol";
...
library L2UpgradeTxLib {
    ...
    /// @dev The inner calldata is IL2V33Upgrade.upgrade() — we decode the placeholder to
    ...
    /// @param _existingUpgradeCalldata The placeholder L2V33Upgrade.upgrade() calldata.
    function buildL2V33UpgradeCalldata(
        address _bridgehub,
        uint256 _chainId,
        bool _zksyncOS,
    ...
    /// @dev The ecosystem-wide transaction wraps `IL2V33Upgrade.upgrade` in
    /// `IComplexUpgrader.forceDeployAndUpgradeUniversal`; only the innermost per-chain field changes, so the
    /// wrapper is unwrapped, `buildL2V33UpgradeCalldata` substitutes the data, and the wrapper is rebuilt.
    /// @param _bridgehub The address of the bridgehub.
    /// @param _chainId The chain ID to build the upgrade data for.
    /// @param _zksyncOS Whether the chain is a ZKsyncOS chain, passed from diamond storage.
    ...
    function rewriteZKsyncOSUpgradeTxData(
        ...
        bytes memory l2UpgradeCalldata = buildL2V33UpgradeCalldata(
            _bridgehub,
            _chainId,
            _zksyncOS,
    ...
    /// @notice Validate that the inner calldata targets L2V33Upgrade.
    function validateWrappedUpgrade(bytes memory _existingUpgradeCalldata) internal pure {
        if (bytes4(_existingUpgradeCalldata) != IL2V33Upgrade.upgrade.selector) {
