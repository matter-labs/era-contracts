...
contract L2SharedBridge is IL2SharedBridge, ILegacyL2SharedBridge, Initializable {
    ...
    IL2NonNativeVault public nonNativeVault;

    /// @dev A mapping l2 token address => l1 token address
    mapping(bytes32 assetIdentifier => address assetHandlerAddress) public override assetHandlerAddress;
    ...
    /// @param _assetHandler The address of the nonNativeVault contract.
    function initialize(
        address _l1SharedBridge,
        address _l1Bridge,
        IL2NonNativeVault _assetHandler
    ...
    function finalizeDeposit(bytes32 _assetIdentifier, bytes memory _data) public override onlyL1Bridge {
        address asset = assetHandlerAddress[_assetIdentifier];
        if (asset != address(0)) {
            IL2AssetHandler(asset).bridgeMint(L1_CHAIN_ID, _assetIdentifier, _data);
        } else {
            IL2AssetHandler(nonNativeVault).bridgeMint(L1_CHAIN_ID, _assetIdentifier, _data);
            assetHandlerAddress[_assetIdentifier] = address(nonNativeVault);
        }

        emit FinalizeDepositSharedBridge(L1_CHAIN_ID, _assetIdentifier, keccak256(_data));
    }

    /// @notice Initiates a withdrawal by burning funds on the contract and sending the message to L1
    /// where tokens would be unlocked
    /// @param _assetIdentifier The L2 token address which is withdrawn
    /// @param _assetData The data that is passed to the asset contract
    function withdraw(bytes32 _assetIdentifier, bytes memory _assetData) public override {
        address asset = assetHandlerAddress[_assetIdentifier];
        ...
        if (asset != address(0)) {
            ...
            _bridgeMintData = IL2AssetHandler(nonNativeVault).bridgeBurn({
                _chainId: L1_CHAIN_ID,
                _mintValue: 0,
                _assetIdentifier: _assetIdentifier,
                _prevMsgSender: msg.sender,
        ...
        bytes memory message = _getL1WithdrawMessage(_assetIdentifier, _bridgeMintData);
        L2ContractHelper.sendMessageToL1(message);

        emit WithdrawalInitiatedSharedBridge(L1_CHAIN_ID, msg.sender, _assetIdentifier, keccak256(_assetData));
    }
    ...
    function _getL1WithdrawMessage(
        ...
        return abi.encodePacked(IL1ERC20Bridge.finalizeWithdrawal.selector, _assetIdentifier, _bridgeMintData);
    }

    /// @dev Used to set the assedAddress for a given assetIdentifier.
    function setAssetHandlerAddress(bytes32 _assetIdentifier, address _assetAddress) external onlyL1Bridge {
        assetHandlerAddress[_assetIdentifier] = _assetAddress;
        emit AssetHandlerRegistered(_assetIdentifier, _assetAddress);
    }
    ...
    function finalizeDeposit(
        ...
        // onlyBridge {
        bytes32 assetIdentifier = keccak256(
        ...
        finalizeDeposit(assetIdentifier, data);
    }
    ...
    function l2TokenAddress(address _l1Token) public view returns (address) {
        return nonNativeVault.l2TokenAddress(_l1Token);
    }
}
