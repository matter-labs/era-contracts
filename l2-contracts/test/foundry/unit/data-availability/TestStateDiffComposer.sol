...
contract TestStateDiffComposer {
    ...
    function appendAMessage(
        bytes memory message, 
        bool includeToArray,
        bool includeToCorrectHash
    ) public {
        if (includeToArray) {
            ++messagesNumber;
    ...
    function appendBytecode(
        ...
        if(includeToArray) {
            ...
            currentRollingBytecodesHash = keccak256(abi.encode(currentRollingBytecodesHash, L2ContractHelper.hashL2BytecodeMemory(bytecode)));
        }
        if (includeToCorrectHash) {
            correctRollingBytecodesHash = keccak256(abi.encode(correctRollingBytecodesHash, L2ContractHelper.hashL2BytecodeMemory(bytecode)));
        }
    }
