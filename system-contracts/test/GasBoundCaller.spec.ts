...
import { GasBoundCallerTesterFactory, type CodeOracleTest, type GasBoundCaller, type GasBoundCallerTester, SystemContext, SystemContextFactory } from "../typechain";
import { REAL_CODE_ORACLE_CONTRACT_ADDRESS, REAL_SYSTEM_CONTEXT_ADDRESS, TEST_GAS_BOUND_CALLER_ADDRESS, TEST_SYSTEM_CONTEXT_CONTRACT_ADDRESS } from "./shared/constants";
...
describe("GasBoundCaller tests", function () {
  ...
  it('Test entry overhead', async () => {
    await (await tester.testEntryOverhead(
    ...
    await (await tester.testEntryOverhead(
        ...
        1000000,
        1000000,
        ethers.utils.randomBytes(100000),
        {
            gasLimit: 80_000_000
        }        
    )).wait();
  ...
  it('test returndata overhead', async () => {
    await (await tester.testReturndataOverhead(
        10,
        {
            gasLimit: 80_000_000
        }        
  ...
  it('Should work correctly if gas provided is enough to cover both gas and pubdata', async () => {
    ...
    await (await tester.gasBoundCall(
        tester.address, 
        80_000_000,
        tester.interface.encodeFunctionData('spender', [0, 100]),
        {
            gasLimit: 80_000_000
        }
    )).wait();
  ...
  it('Should work correctly if gas provided is not enough to cover both gas and pubdata', async () => {
    ...
    await expect((await tester.gasBoundCallRelayer(
        gasSpentOnPubdata,
        tester.address, 
        gasSpentOnPubdata,
  ...
  it('Should work correctly if maxGasToUse permits large pubdata usage', async () => {
    ...
    await (await tester.gasBoundCallRelayer(
        gasSpentOnPubdata,
        tester.address, 
        80_000_000,
        tester.interface.encodeFunctionData('spender', [0, pubdataToSend]),
