...
import { L1Messenger } from "../typechain";
...
describe("L1Messenger tests", () => {
  ...
  describe("requestBytecodeL1Publication", async () => {
    ...
    describe("should clear l1Messenger state", async () => {
      it("should clear l1Messenger state", async () => {
        // should revert Too many L2->L1 logs
      const isService = true;
      const key = Buffer.alloc(32, 1);
      const value = Buffer.alloc(32, 2);
      const txNumberInBlock = 1;
      ...
      const fourthLog = ethers.utils.concat([
        ethers.utils.hexlify([0]),
        ethers.utils.hexlify([false ? 1 : 0]),
        ethers.utils.hexZeroPad(ethers.utils.hexlify(txNumberInBlock), 2),
        ethers.utils.hexZeroPad(l1MessengerAccount.address, 20),
        key,
        value,
      ...
      await l1Messenger.connect(bootloaderAccount)
      .publishPubdataAndClearState(totalL2ToL1PubdataAndStateDiffs)
      });
