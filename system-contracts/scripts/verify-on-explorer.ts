...
import { SYSTEM_CONTRACTS, YulContractDescription } from "./constants";
...
async function waitForVerificationResult(requestId: number) {
    ...
    while (true) {
        ...
        let statusObject = await query('GET', `${VERIFICATION_URL}/${requestId}`);
...
async function verifyYul(contractInfo: YulContractDescription) {
    ...
    let requestId = await query('POST', VERIFICATION_URL, undefined, requestBody);
...
async function main() {
  ...
  program.version("0.1.0").name("verify on explorer").description("Verify system contracts source code on block explorer");
