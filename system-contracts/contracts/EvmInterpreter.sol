...
contract EvmInterpreter {
    ...
    */
    ...
    uint256 constant STACK_OFFSET = 32;
    uint256 constant BYTECODE_OFFSET = 1024 + 32;

    function _getBytecode(bool _isCallerEVM) returns (uint256 _evmGas) {
        ...
        assembly {
            ...
            default {
                ...
                // EVM provides gas as first of its param
                returndatacopy(
                    0,
                    0,
                    32
    ...
    function _setDeployedCode(
        ...
        require(offset > 64, "Offset too small");
    ...
    constructor() {
        ...
        if (_isEVM(msg.sender)){
            evmGas = _getBytecode(true);
        } else {
            _getBytecode(false);
            evmGas = _getEVMGas();
        }

        (uint256 offset, uint256 len) = _simulate(msg.data[0:0], evmGas, _bytecodeLen);
    ...
    function _simulate(bytes calldata input, uint256 gasLeft, uint256 _bytecodeLen) internal returns (uint256 offset, uint256 len){
        // unchecked {
    ...
    function _requestBytecode(address _addr) internal view returns (bytes memory bytecode) {
    ...
    fallback() external payable {
        ...
        if(msg.sender == address(DEPLOYER_SYSTEM_CONTRACT)) {
            ...
            evmGas = uint256(bytes32(msg.data[0:32]));
            input = msg.data[32:];
            bytecode = _requestBytecode(address(this));
        } else {
            evmGas = _getEVMGas();
            input = msg.data;
        }
