...
contract DIMTLegacyTester {
     ...
     */
    function calculateLegacyMerkleRoot(bytes32[] memory leaves) public pure returns (bytes32) {
        ...
        uint256 nodesOnCurrentLevel = leaves.length;
        ...
        while (nodesOnCurrentLevel > 1) {
            ...
            for (uint256 i = 0; i < nodesOnCurrentLevel; i++) {
                workingArray[i] = keccak256(
                    abi.encode(workingArray[2 * i], workingArray[2 * i + 1])
                );
            }
        }
     ...
     */
    function calculateDIMTRoot(bytes32[] memory leaves) public pure returns (bytes32) {
        if (leaves.length == 0) {
            return bytes32(0);
        }
     ...
     */
    function testEquivalence(bytes32[] memory leaves) external pure returns (bool equivalent, bytes32 legacyRoot, bytes32 dimtRoot) {
     ...
     */
    function batchTestEquivalence(bytes32[][] memory leafSets) external pure returns (
        bool[] memory equivalences,
        bytes32[] memory legacyRoots,
        bytes32[] memory dimtRoots
    ) {
