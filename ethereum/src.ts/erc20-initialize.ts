...
export async function initializeErc20Bridge(
    ...
    const { l2TokenFactoryAddr, l2ERC20BridgeProxyAddr } = calculateERC20Addresses(l2ProxyAdminAddress, l2GovernorAddress, erc20Bridge);
  ...
  export async function startInitializeChain(deployer: Deployer, deployWallet: Wallet, chainId: string,  nonce:number, gasPrice: ethers.BigNumberish, erc20BridgeAddress? : string,) {
    const bridgehub = deployer.bridgehubContract(deployWallet);
