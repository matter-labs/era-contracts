...
import "../bridgehub-interfaces/IBridgehubAdmin.sol";
...
contract BridgehubAdminFacet is BridgehubBase, IBridgehubAdmin {
    string public constant override getName = "BridgehubAdminFacet";
    ...
    function setPendingGovernor(address _newPendingGovernor) external onlyGovernor {
        ...
        address oldPendingGovernor = bridgehubStorage.pendingGovernor;
        // Change pending governor
        bridgehubStorage.pendingGovernor = _newPendingGovernor;
    ...
    function acceptGovernor() external {
        address pendingGovernor = bridgehubStorage.pendingGovernor;
        require(msg.sender == pendingGovernor, "n4"); // Only proposed by current governor address can claim the governor rights

        address previousGovernor = bridgehubStorage.governor;
        bridgehubStorage.governor = pendingGovernor;
        delete bridgehubStorage.pendingGovernor;
    ...
    function setPendingAdmin(address _newPendingAdmin) external onlyGovernorOrAdmin {
        // Save previous value into the stack to put it into the event later
        address oldPendingAdmin = bridgehubStorage.pendingAdmin;
        // Change pending admin
        bridgehubStorage.pendingAdmin = _newPendingAdmin;
        emit NewPendingGovernor(oldPendingAdmin, _newPendingAdmin);
    }

    /// @notice Accepts transfer of admin rights. Only pending admin can accept the role.
    function acceptAdmin() external {
        address pendingAdmin = bridgehubStorage.pendingAdmin;
        require(msg.sender == pendingAdmin, "n4"); // Only proposed by current admin address can claim the admin rights

        address previousAdmin = bridgehubStorage.admin;
        bridgehubStorage.admin = pendingAdmin;
        delete bridgehubStorage.pendingAdmin;
