...
contract ExecutorFacet is StateTransitionChainBase, IExecutor {
    ...
    function _commitBatchesWithSystemContractsUpgrade(
        ...
        // While the logic of the contract ensures that the chainStorage.l2SystemContractsUpgradeBatchNumber is 0 when this function is called,
        // this check is added just in case. Since it is a hot read, it does not encure noticable gas cost.
        require(chainStorage.l2SystemContractsUpgradeBatchNumber == 0, "ik");
    ...
    function _executeOneBatch(StoredBatchInfo memory _storedBatch, uint256 _executedBatchIdx) internal {
        ...
        require(currentBatchNumber == chainStorage.totalBatchesExecuted + _executedBatchIdx + 1, "k"); // Execute batches in order
        require(
            _hashStoredBatchInfo(_storedBatch) == chainStorage.storedBatchHashes[currentBatchNumber],
    ...
    function proveBatches(
        ...
        require(_hashStoredBatchInfo(_prevBatch) == chainStorage.storedBatchHashes[currentTotalBatchesVerified], "t1");
        ...
        require(currentTotalBatchesVerified <= chainStorage.totalBatchesCommitted, "q");
    ...
    function revertBatches(uint256 _newLastBatch) external nonReentrant onlyValidator {
        require(chainStorage.totalBatchesCommitted > _newLastBatch, "v1"); // The last committed batch is less than new last batch
        require(_newLastBatch >= chainStorage.totalBatchesExecuted, "v2"); // Already executed batches cannot be reverted

        if (_newLastBatch < chainStorage.totalBatchesVerified) {
            chainStorage.totalBatchesVerified = _newLastBatch;
        }
        chainStorage.totalBatchesCommitted = _newLastBatch;

        // Reset the batch number of the executed system contracts upgrade transaction if the batch
        // where the system contracts upgrade was committed is among the reverted batches.
        if (chainStorage.l2SystemContractsUpgradeBatchNumber > _newLastBatch) {
            delete chainStorage.l2SystemContractsUpgradeBatchNumber;
        }
