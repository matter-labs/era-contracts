...
/// Reads a config from the specified path.
/// If the file does not exist, it generates a default config using `default_gen`,
/// saves it to the path, and returns it.
pub fn get_or_create_config<T, F>(
    shell: &Shell,
    path: impl AsRef<Path>,
    default_gen: F,
) -> anyhow::Result<T>
where
    T: ReadConfig + SaveConfig,
    F: FnOnce() -> T,
{
    ...
    if path.exists() {
        ...
        // File doesn't exist, create default, save, and return
        let default_config = default_gen();
        default_config.save(shell, path).with_context(|| {
            format!("Failed to save new default config to {:?}", path)
        })?;
        Ok(default_config)
    }
}
