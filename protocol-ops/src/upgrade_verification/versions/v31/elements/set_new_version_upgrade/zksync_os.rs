...
//! L2V33Upgrade delegate target, validated separately), the
//! deployed-bytecode-info decoder (96-byte triple or 320-byte impl/proxy
//! pair), the keccak-derived L2V33Upgrade delegate-address check, the ZKsync
...
/// `ComplexUpgrader.forceDeployAndUpgradeUniversal` — excludes the L2V33Upgrade delegate-target
...
fn expected_v31_zksync_os_force_deployments() -> Vec<ZksyncOSExpectedFd> {
    ...
    // ZKsyncOS force deployment except the L2V33Upgrade delegate target (validated separately);
...
/// Validate all entries of `UniversalContractUpgradeInfo[]` except the L2V33Upgrade delegate-target
...
fn verify_v31_zksync_os_force_deployments(
    ...
    for deployment in deployments {
        // Skip the L2V33Upgrade delegate-target; already validated elsewhere. It is the ONLY
            ...
            == IComplexUpgrader::ContractUpgradeType::ZKsyncOSUnsafeForceDeployment
        {
            result.report_error(&format!(
                "Unsafe ZKsyncOS force deployment at {} is not allowed (only the L2V33Upgrade \
    ...
    if missing.is_empty() {
        result.report_ok(
            "All ZKsyncOS force deployments match the expected v31 list (excluding L2V33Upgrade delegate target)",
        );
    }
}
...
/// via address derivation (unlike the L2V33Upgrade delegate target), so PUVT must
...
/// `CoreOnGatewayHelper.getFullListOfFactoryDependencies(true, [L2V33Upgrade])`.
...
/// L2V33Upgrade delegate-target entry (derived address + bytecode info), then decodes
/// the inner `IL2V33Upgrade.upgrade` calldata.
pub(super) async fn verify_zksync_os_force_deploy_and_upgrade(
    ...
    // Validate all expected force deployments (18 fixed entries; L2V33Upgrade delegate validated below).
    ...
    // Validate the L2V33Upgrade delegate-target entry (1 unsafe force deployment at a derived address).
...
fn verify_zksync_os_l2_v31_deployment(
    ...
    {
        result.report_error(&format!(
            "ZKsync OS L2V33Upgrade deployment must use ZKsyncOSUnsafeForceDeployment, got {:?}",
    ...
    match zksync_os_bytecode_info_hashes(&deployment.deployedBytecodeInfo) {
        Some((first_hash, observable_hash)) => {
            if evm_deployed_bytecode_hash_matches_file(
                verifiers,
                &observable_hash,
                L2_V33_UPGRADE_CONTRACT,
            ) {
                result.report_ok("ZKsync OS delegate deployment uses L2V33Upgrade bytecode info");
            } else {
                result.report_error(&format!(
                    "ZKsync OS delegate bytecode info does not map to {}: blake={}, observable={}",
                    L2_V33_UPGRADE_CONTRACT, first_hash, observable_hash
                ));
            }
        }
        None => result.report_error(&format!(
            "ZKsync OS L2V33Upgrade bytecode info must be 96 bytes, got {}",
