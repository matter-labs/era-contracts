...
use crate::{
    ...
    commands::ecosystem::verify_v31_stage_patch::VerifyV31StagePatchArgs,
};
...
pub(crate) mod verify_v31_stage_patch;
...
pub enum EcosystemCommands {
    ...
    /// Verify the narrow v31 stage emergency patch calldata and implementation deployment.
    #[command(name = "verify-v31-stage-patch")]
    VerifyV31StagePatch(VerifyV31StagePatchArgs),
...
pub(crate) async fn run(args: EcosystemCommands) -> anyhow::Result<()> {
    match args {
        ...
        EcosystemCommands::VerifyV31StagePatch(args) => verify_v31_stage_patch::run(args).await,
