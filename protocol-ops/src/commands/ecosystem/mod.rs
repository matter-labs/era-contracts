...
pub enum EcosystemCommands {
    ...
    /// Phase 3 of the ecosystem upgrade: bridged-token registration. Calls
    /// `CoreUpgrade_v31.stage3(bridgehub)`, which registers ETH + every
    /// v31-bridged token in the NTV's bridgedTokens list. Runs *before*
    /// per-chain upgrades so the NTV `_requireRegistered` gate is cleared
    /// before each chain's diamond upgrade lands. Any signer.
    Stage3(Stage3Args),
