...
use crate::common::traits::{ReadConfig, SaveConfig};
use crate::common::wallets::Wallet;
use crate::common::forge::{Forge, ForgeRunner, ForgeScriptArgs};
...
/// Validate that a private key matches an expected address.
pub fn validate_pk_address(pk: H256, expected: Address) -> anyhow::Result<()> {
    let wallet = LocalWallet::from_bytes(pk.as_bytes())
        .map_err(|e| anyhow::anyhow!("Invalid private key: {}", e))?;
    if wallet.address() != expected {
        anyhow::bail!(
            "Private key does not match address: got {:#x}, want {:#x}",
            wallet.address(),
            expected
        );
    }
...
/// Resolve owner auth from optional owner/owner_private_key args.
///
/// - If owner == sender, reuse sender_auth
/// - If simulation mode, use Unlocked(owner)
/// - Otherwise, require owner_private_key and validate it
pub fn resolve_owner_auth(
    owner: Address,
    owner_private_key: Option<H256>,
    sender: Address,
    sender_auth: &SenderAuth,
    is_simulation: bool,
) -> anyhow::Result<SenderAuth> {
    if owner == sender {
        return Ok(sender_auth.clone());
    }
    if let Some(pk) = owner_private_key {
        validate_pk_address(pk, owner)?;
        Ok(SenderAuth::PrivateKey(pk))
    } else if is_simulation {
        Ok(SenderAuth::Unlocked(owner))
    } else {
        anyhow::bail!(
...
/// Resolve an optional secondary auth (bridgehub admin, governance owner, etc.)
/// Falls back to the provided default if no key is given.
pub fn resolve_secondary_auth(
    private_key: Option<H256>,
    label: &str,
    fallback: &SenderAuth,
) -> anyhow::Result<SenderAuth> {
    match private_key {
        Some(pk) => {
            let wallet = LocalWallet::from_bytes(pk.as_bytes())
                .map_err(|e| anyhow::anyhow!("Invalid {} private key: {}", label, e))?;
            crate::common::logger::info(format!("{}: {:#x}", label, wallet.address()));
            Ok(SenderAuth::PrivateKey(pk))
        }
