...
> **On this branch:** the `regen-and-verify-stage.sh` wrapper this skill drives was removed with
> the EraVM cut. Its steps survive as protocol-ops subcommands (`ecosystem upgrade-prepare-all`,
> `ecosystem verify-upgrade`), so the two-machine loop below still holds — the single command that
> chained them for the v31 stage env does not, and has to be driven step by step until a replacement
> lands. Everything about macOS/Linux artifact divergence is unaffected.
