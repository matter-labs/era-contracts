...
> **On this branch:** two helpers this skill invokes were removed with the EraVM cut —
> `regen-and-verify-stage.sh` and `scripts/regen-via-docker.ts`. The underlying protocol-ops
> subcommands (`ecosystem upgrade-prepare-all`, `ecosystem verify-upgrade`) are unchanged, but the
> Mode 1 / Mode 2 Docker flows described below have no driver script and need rework before this
> skill can be followed end to end.
...
rebuild. Prepare runs forge with `--offline`, which skips the post-success
`ExternalIdentifier` lookup outright — that is what keeps it under 15 min per
CTM. The old `--add-host sourcify.dev:127.0.0.1` container hack existed only
because foundry-zksync silently ignored `--disable-labels` for `forge script`;
upstream forge honours `--offline`, so the hack is gone (see `with_offline` in
`protocol-ops/src/common/forge/script.rs`).
