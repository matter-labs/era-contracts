...
cross-chain messaging, dormant settlement-layer machinery, consensus registry, and the two supported
execution environments. The focused documents linked below remain the source of truth for their
individual flows.
...
flowchart TB
    ...
    D --> EX[Executor facet]
    D --> MB[Mailbox facet]
    D --> AD[Admin and getters facets]
    EX --> V[Verifier]
    EX --> DA[L1 DA validator]
    MB --> PQ[Priority queue or tree]
    ...
    BL[Bootloader] --> SC[EraVM system contracts]
    BL --> LM[L1Messenger and pubdata]
    LM --> DA
```
...
| Chain settlement            | The diamond's executor validates batch commitments, DA output, and proofs before execution. The mailbox manages priority operations and L1 <-> L2 messages. Admin/getter facets provide controlled mutation and inspection.                       | [Settlement contracts](./contracts/settlement_contracts/zkchain_basics.md), [priority operations](./contracts/settlement_contracts/priority_queue/README.md) |
| Data availability           | L2 publishes canonical pubdata through `L1Messenger`; an L2 DA validator commits to it and the configured L1 DA validator checks the operator input. Rollup, validium, and custom DA validators share this interface.                             | [Data availability](./contracts/settlement_contracts/data_availability/README.md)                                                                            |
...
| Settlement layers / Gateway | The generic relay, migration, and nested-proof code remains in the repository, but production chain migrations are disabled in this release. All supported chains settle on L1.                                                                   | [Current release status](./contracts/gateway/README.md), [chain lifecycle](../chain-lifecycle.md#settlement-layer-migration-chainassethandler)               |
...
| Execution environment       | EraVM chains use the bootloader and kernel-space system contracts. ZKsync OS chains use a distinct execution environment but share the L1 coordination, settlement, bridging, message-root, and interop contracts where their interfaces overlap. | [EraVM / zkEVM](./contracts/zkevm/overview.md), [ZKsync OS genesis](../chain-lifecycle.md#zksync-os-genesis-force-deployments-atomic-interop-built-ins)      |
...
- **Settlement layers** would become an additional trust and liveness boundary for migrated chains. That
  path is intentionally unavailable while `CHAIN_MIGRATIONS_ENABLED` is false.
...
1. Users submit L2 transactions; L1 -> L2 priority operations enter through Bridgehub/Mailbox and are
   committed to the chain's priority queue or tree.
2. The bootloader executes a batch and the L1 messenger commits its logs, messages, bytecodes, and state
   differences as pubdata.
3. The operator commits the batch on the executor facet. The configured L1 DA validator checks the DA
...
7. [EraVM system contracts](./contracts/zkevm/overview.md)
