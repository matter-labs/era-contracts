...
- **No trusted coordinator.** There is no L1 atomic contract, global-root registry, or L2->L1 message;
  finality and timeout are both proven against the ordinary interop-root channel.
...
- **Native-`value` legs are rejected at send.** A raw value transfer has no recoverable sender, so it
  could never be reversed on timeout.
...
1. **Every chain interop can target has at least one batch inside the settlement layer's message root.**
   ...
   - Freshly created chains report their genesis batch root right after registration, in the same
     ...
     `MessageRoot.seedGenesisRoot`, which pulls it from the chain's getters (once-only, fresh ZKsync OS
     chains only). See {protocol-docs/message-root.md#genesis-root-and-seedgenesisroot}.
   - Already-deployed chains onboarded with a non-zero starting batch number are NOT seeded (a real
     ...
     batch into the message root (`MessageRoot.chainTreeLeafCount > 0`).
...
3. **Every batch leaf carries the timestamp at which it entered the shared root.** Enforced by
   `MessageRoot.addChainBatchRootV32`, which folds the settlement-layer `block.timestamp` into the batch
   leaf (`MessageHashing.batchLeafHash`); the timestamp (`t`) is therefore proven by the same inclusion
   proof that authenticates the IMT root. Aggregated roots additionally carry their own creation
   timestamp (`T`) — one `(blockNumber, root, timestamp)` tuple exposed by `MessageRoot.historicalRoot`
   on the settlement layer, imported into `L2InteropRootStorage.interopRoots` and double-checked at
   batch execution — which anchors the "root from after the deadline" requirement. See the two clocks in
...
Because the IMT root travels through the chain batch root and the proofs must be reconstructible from L1
data, atomic-interop chains use the `L2_TO_L1_ONLY` DA scheme (`CalldataL1DAValidatorZKsyncOS`), which
publishes the L2->L1 region as permanent L1 calldata — enough to keep the IMT contents (hence every
inclusion / non-inclusion proof) reconstructible from L1 data alone.
