...
The manager is agnostic to call/encoding formats — it forwards `(destChainId, data)` and lets each
sender own its reversal. Senders **must** return `false` rather than revert for calls they do not
recognize, so one unrecognized call cannot brick the whole refund.
...
`L2AssetRouter.recoverAtomicCall` and `L2AssetRouter.bridgehubRecoverBaseToken`. Atomic L2->L1 bundles
are already rejected at send time, so this is belt-and-suspenders — but a load-bearing one: it keeps
recovery from ever reaching the append-only L1 deposit/withdrawal counters in `L2AssetTracker`, whose
settlement-layer-conditional updates are only correct when evaluated at send time, not recovery time.
The full accounting reason is in {protocol-docs/bridging.md#security-notes} ("L2 -> L1 withdrawals are
never revertable").
...
- **Native-`value` legs are rejected at send**, because a value transfer with no recoverable sender
  could never be reversed. See {protocol-docs/atomicity/security.md#non-guarantees}.
