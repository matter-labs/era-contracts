...
- `insert(value, lowLeafIndex)`: the caller supplies the index of the **low leaf** (the leaf expected to precede `value` in sorted order). Zero and duplicate values are rejected. If the hint is stale, the function walks the linked list forward until it reaches the true low leaf. The walk is deliberately **unbounded**: hints are computed off-chain against a snapshot of the tree, and values are keccak digests, so a griefer can grind commit values into the gap between a pending insert's hint and its value — with any fixed hop bound, landing that many front-run inserts in the gap would revert the insert. Because the tree is append-only and leaf values never change, a once-valid hint stays a valid starting point forever, so an insert with a once-valid hint can no longer hit a hint-validity revert; the only residual failure is out-of-gas, if enough front-run inserts land in the gap within one send's lifetime. That grief is strictly uneconomical and self-defeating: every extra hop (a few storage reads for the caller) must first be paid for by the griefer with a full insert of its own, the caller can raise its gas limit (each hop is a small, bounded cost), and a failed send retried with a freshly computed hint restarts with an empty gap — the griefer must re-pay the entire gap for every attempt while the caller only re-pays one transaction. Repeated attempts also cost the griefer exponentially more grinding work: each retry's fresh hint is the value's current predecessor, so the griefer's own previous inserts have shrunk the interval its next commit values must land in — every round multiplies the expected keccak search per value by the previous round's shrink factor, i.e. the off-chain grinding cost grows exponentially in the number of repeated attacks. The low leaf's `nextIndex`/`nextValue` are re-pointed at the new leaf, which inherits the old successors; the tree is append-only (leaves are never removed).
