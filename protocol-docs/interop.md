...
- **Destination-side value on an indirect call is refunded to the sender contract**, since recovery returns native value to the call's `InteropCall.from` — for an indirect call the sender contract (e.g. the asset router) rather than the payer; reversing it is that sender's `recoverAtomicCall` job. Direct calls may carry value — on timeout it is refunded via `bridgehubRecoverBaseToken`. Everything else is allowed (an atomic bundle may mix recoverable fund calls, e.g. asset-router deposits, with fund-free calls); timeout recovery is best-effort and refund safety of fund-moving legs is the flow author's responsibility.
...
- **Timeouts**: if a flow misses its deadline, recovery is best-effort via `AtomicFlowManager._recoverBundle` / `IAtomicRecoverable.recoverAtomicCall`. This is why destination-side value on an _indirect_ call is the sender contract's own problem to reverse (see [Restrictions](#restrictions)) — recovery returns value to `InteropCall.from`, the sender contract for an indirect call, never the payer; direct calls carry their value themselves and are refunded through `bridgehubRecoverBaseToken`. L1 destinations are banned because L1 has no atomic execution, so the only outcome would be a timeout refund, but L2→L1 withdrawal accounting (`totalWithdrawalsToL1`, consumed once during the L1→GW migration) must stay append-only and never revertable.
