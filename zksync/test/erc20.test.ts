...
describe("ERC20Bridge", function () {
  ...
  before("Deploy token and bridge", async function () {
    ...
    const l2Erc20TokenBeacon = await deployer.deploy(await deployer.loadArtifact("UpgradeableBeacon"), [l2TokenImplAddress.address]);
  ...
  it("Should finalize deposit ERC20 deposit", async function () {
    ...
    const tx = await (await erc20BridgeWithL1Bridge.finalizeDeposit(
  ...
  it("Governance should be able to reinitialize the token", async () => {
    ...
    await (await erc20TokenWithGovernor.reinitializeToken(
        {
            ignoreName: false,
            ignoreSymbol: false,
