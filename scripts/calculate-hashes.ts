...
const getBytecodeHashFromEvmJson = (jsonFileContents: any) => {
    try {
        if (jsonFileContents.deployedBytecode.object == "0x") {
            return ["0x", "0x"];

        }
        return [ethers.utils.hexlify(ethers.utils.keccak256(ethers.utils.arrayify(ethers.utils.hexlify(jsonFileContents.bytecode.object)))),
        ethers.utils.hexlify(ethers.utils.keccak256(ethers.utils.arrayify(ethers.utils.hexlify(jsonFileContents.deployedBytecode.object))))
        ];
...
const getYulContractDetails = (dir: string, contractName: string): ContractsInfo => {
    const zkBytecodePath = join(dir, YUL_ARTIFACTS_DIR, contractName + ".yul.zbin");
    ...
    return {
        ...
        zkBytecodePath,
        zkBytecodeHash: ethers.utils.hexlify(hashBytecode(fs.readFileSync(zkBytecodePath))),
