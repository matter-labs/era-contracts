...
contract DeployL2Script is Script {
    ...
    function initializeConfig() public {
        ...
        config.bridgehubAddress = toml.readAddress(
            "$.bridgehubAddress"
        );
        config.governance = toml.readAddress(
            "$.governance"
        );
        config.l1SharedBridgeProxy = toml.readAddress(
            "$.l1sharedBridgeProxy"
        );
        config.chainId = toml.readUint(
            "$.chainId"
        );
        config.erc20BridgeProxy = toml.readAddress(
            "$.erc20BridgeProxy"
        );
    }
    ...
    function deployFactoryDeps() public {
        ...
        bytes memory l2StandartErc20FactoryBytecode = readHardheadBytecode("/../l2-contracts/artifacts-zk/@openzeppelin/contracts/proxy/beacon/UpgradeableBeacon.sol/UpgradeableBeacon.json");
        bytes memory beaconProxy = readHardheadBytecode("/../l2-contracts/artifacts-zk/@openzeppelin/contracts/proxy/beacon/BeaconProxy.sol/BeaconProxy.json");
        bytes memory l2StandartErc20Bytecode = readHardheadBytecode("/../l2-contracts/artifacts-zk/contracts/bridge/L2StandardERC20.sol/L2StandardERC20.json");
    ...
    function deploySharedBridge() public {
        // HACK: We use the bytecode builded by hardhat to deploy the contracts
        bytes memory l2SharedBridgeBytecode = readHardheadBytecode("/../l2-contracts/artifacts-zk/contracts/bridge/L2SharedBridge.sol/L2SharedBridge.json");
        bytes memory beaconProxy = readHardheadBytecode("/../l2-contracts/artifacts-zk/@openzeppelin/contracts/proxy/beacon/BeaconProxy.sol/BeaconProxy.json");
    ...
    function deploySharedBridgeProxy() public {
        ...
        bytes memory l2SharedBridgeProxyConstructorData = abi.encode(
            ...
            config.l2SharedBridgeImplementation, l2GovernorAddress, proxyInitializationParams
        );

        /// loading TransparentUpgradeableProxy bytecode
        bytes memory l2SharedBridgeProxyBytecode = readHardheadBytecode("/../l2-contracts/artifacts-zk/@openzeppelin/contracts/proxy/transparent/TransparentUpgradeableProxy.sol/TransparentUpgradeableProxy.json");
